...
static void test_current_roundtrip(void)
{
    ...
    u32b sentinel; size_t consumed;
    assert(fixture_read_dungeon(encoded, length, 6, &sentinel, &consumed) == 0);
    ...
    puts("Real .6 dungeon roundtrip: normalized ages, ice/poison/lava/water, monster AI/history/recovery, following sentinel PASS.");
    ...
    {
        fresh_map();
        assert(fixture_read_dungeon(encoded, truncations[n], 6, &sentinel, &consumed) != 0);
    }
    /* Re-encode the changed decoded byte, preserving the rest of the stream. */
    plain[scent_start] = 0;
    encode(plain, modified, length); fresh_map();
    assert(fixture_read_dungeon(modified, length, 6, &sentinel, &consumed) != 0);
    ...
    puts("Real .6 reader: five scent header/grid truncations, malformed magic and out-of-range age rejected PASS.");
}
...
def main():
    ...
    harness += '    test_current_roundtrip(); test_legacy_absence();\n'
