...
Water/acid cross 100 corridor tiles in 5/3/2/2 actions at anger 0/2/4/6. They affect 90% of a 50×50 open chamber in 39/16/11/9 actions; lava/ice need 74/27/18/14, chasm 144/51/32/24. At anger 6, normal-source water reaches 90% of dry ground in 9–35 actions on 10/15 generated maps. Same-source five-map median coverage at action 40: water 98.5%, acid 63.4%, lava 20.4%, ice 0.7%, chasm 17.2%. Ice's closed-door barriers dominate its results. The queue lets newly converted cells propagate again within the same action, so area quotas permit extreme corridor travel; the eight-step ramp and anger multiplier compound it. Recommend testing a per-action limit on new tile layers before uniformly reducing material rates. No gameplay implementation changed. Full methods, sample caveats, CSVs and charts: `scripts/output/catastrophe-speed/report.md`.

## 2026-09-26: Catastrophe event/revelation repair

- Deployment log reproduced the water catastrophe at `(64,138)`, depth 16: it continued emitting a source-position flood event after `changed=0`. Catastrophes now emit only on actual new terrain/deepening/bridge collapse and locate the event on a changed front cell. No fake repeated flood at a stopped, full or lost source. Diagnostic step logging includes deepening, reachable cells, actual source terrain and activity coordinates.
- Debug menus match keys without case sensitivity, so `W` shadowed `w`. Catastrophe moved to `!`; regression uses the real menu input matcher for `!`, `w` and `d`. Both reveal commands explicitly refresh terrain and bridge-underlay memory and invalidate retained map textures. Revelation exposes floors and boundary walls, clears accidentally revealed buried wall interiors, and does not grant permanent sight.
- Read-only inspection of a temporary copy of the supplied Glorfindel save found step 133, 2,562 actual/remembered deep-water cells, 2,568 reached cells including protected support, and zero stale feature memories after revelation. All 2,562 water cells differ from generation terrain. The source is deep water, and its front has no new ground to flood. A production SDL software-rendered reconstruction confirms the flooded region on the right; this is not an absent catastrophe. Original save was not edited. Diagnostic files are under ignored `scripts/output/catastrophe/`.
- Validation: Windows build; 2,148 catastrophe checks including moving/silent front events, old memory refresh, actual map glyphs, bridge underlays, real hotkeys and buried-wall filtering; 41,482 ordinary environment checks; 1,575 save checks; water-flood/SDL rendering regressions. No save-format change.

## 2026-09-26: Monster physical-action sound coverage

- Successful monster door opening/unlocking and bashing already use positional `MSG_OPENDOOR`/`MSG_BASHDOOR`; Song of Binding closes doors through `lock_door()`, which uses positional `MSG_SHUTDOOR`. Added positional `MSG_BASHDOOR_FAIL`/`MSG_LOCKPICK_FAIL` for failed movement-time door attempts.
- Reused existing audio for monster-broken wards (`MSG_HITWALL`), item pickup (`MSG_PICK`), crushed floor objects (`MSG_HITWALL`), and ambient monster-versus-monster deaths (`MONSTER_SOUND_DEATH`). Door pass-through and wall phasing remain silent because they do not alter the door or wall.
- Validation: incremental Windows build, monster sound assignment/OGG harness, world-sound harness (182 checks), and monster AI combat harness (87 checks) passed. Monster social persistence harness still has its unrelated `MON_SOCIAL_SAVE_MAGIC` assertion failure; no interactive listening was performed.

## 2026-09-26: Morgoth's Wrath implementation

- `cave/cave-catastrophe.c` controls persistent descent probability, committed event rolls, independent forged/acquired artefact milestone masks, stable acquired GUIDs, and immediate action-driven water/acid/lava/ice/chasm fronts. Escape levels initialize automatically, including after Morgoth's death; generation supplies a connected natural spring if necessary. Ruined/labyrinth partitions are targets, never geological causes.
- Event hooks run after actual kills, rewards, handovers, permanent attribute purchases, successful possession, completed songs and crafting. The raw minimum-depth cycle continues beyond 1,000 ft and announces each qualifying roll. Craft difficulty is captured when a job is accepted and survives interruption/save; old interrupted jobs initialize it when resumed.
- Shared flood corner rules and environment conversion preserve normal flood/ecology behavior. Wrath bypasses occupied-cell and route vetoes while retaining permanent tiles, stairs, forges and Crown/Silmaril support. Structure contact occurs once per action independently of new-cell quota. Connected live bridge underlays, material wear, cold/contact/fall handlers, immediate thermal reactions, deepening and saved cell ages govern propagation. Closed doors block ice; water cannot breach sound rock or stone bridges.
- `Catastrophe` appears directly below the timer in SDL and compact sheets, with a shared description. Ctrl+Y > Map and Travel > Trigger catastrophe (!) supports automatic and available material choices; an already-active catastrophe reports its type/origin. Save version is 0.9.8.23; the append-only dungeon tail persists controller RNG, milestones, pending events, accepted difficulty, acquired identities and RLE cell ages. Loading .22 saves does not misread the new tail or replay old deeds.
- Validation: Windows incremental build; 1,697 focused production-linked checks; 41,482 existing Living Dungeon checks; 1,575 existing environment/save checks; existing water flood, lava, poison and ice combat regressions. The focused suite verifies exact next-map/cell/RNG save continuation, legacy .22 loading, genuine unique kills/song wins, timer messages, quiver possession, threshold boundaries, material interactions, debug handling, SDL vital rows and compact terminal output at 80/60/40 columns.
- `python scripts/check_catastrophe.py --levels` runs 12 production-generated map replays (250/500/750 ft, anger 0/2/4/6, 40 completed actions). Chasm changed 962/2115/3228/4404 cells; water 2342/5151/5151/5151; acid 2868/7502/12113/17002. The measured acid dry route disappeared at action 34/25 for anger 4/6; permanent boundaries and stairs survived. Output: `scripts/output/catastrophe/levels.log`. These automated terrain/hazard replays keep an observer on a stair and do not constitute human escape balance testing or a full monster-scheduler replay. SDL visual interaction remains unverified; its collector and compact terminal output were exercised.
...
The target is an observable cause-and-resolution sequence during ordinary play, with limited simultaneous obstruction and permanent damage. Measure **changes per visit, completion of each stage, effect lifetime, maximum affected area, and activity on a later visit** separately. A quiet snapshot alone cannot distinguish successful clearing from an inert simulation. Short physical reactions, recurring flows, permanent structural damage and resource creation should not share one universal slowdown.

| Effect | Analysis and current rule |
| --- | --- |
| Chasm walls | Whole-bank synchronized erosion made rubble rings. The first attempted fix randomly scattered weak damage so almost nothing finished. Now one exposed weak point receives continuing wear until it cracks and collapses; a new intact failure cannot start within two cells of unsettled rubble. Source pulses are 30 Normal actions; first pulses start after 15-30. All current samples complete a crack/collapse/clear sequence inside 200 actions, with one rubble pile at a time. Permanent rock removal can eventually settle down after a finite bank has failed; it should not manufacture infinite fresh rock. |
| Chasm widening | Different from an edge rockfall: a new permanent hole gets a one-in-six opportunity per pulse and retains its small lifetime expansion limit. Settled ledges are not repeatedly turned into new holes. |
| Rubble | Loose debris at a drop or moving/corrosive/hot liquid clears independently of unused chasm capacity and natural-partition classification. Base exposure is 30-45 Normal actions, plus scheduling; measured new piles last about 39-41 actions. Sheltered rubble remains. This represents fragments leaving a passable ledge, not dry rock evaporating. Existing old rubble also benefits. |
| Water | Restored the original 10-Normal-action flow interval. Six rising stages alternate with six receding stages. Small reservoirs change at most two added cells at a time; larger ones retain a bounded capacity up to 12. Clearing/recession returns water supply. No erosion of granite by a calm pool. |
| Acid | Flows on water's 10-action cadence: corrosiveness is not a reason for a liquid to move much more slowly. Its smaller footprint limit and stronger, focused corrosion distinguish it. Contact damage is unchanged. No unsupported acid/water neutralization formula was introduced. |
| Lava | Restored the original 350-tick Slow interval (about 20 actions at Normal after world-pulse quantization). Four surge stages and eight cooling stages. A source repeatedly reheats its already reserved cooled fringe, rather than either paving unlimited new terrain or spending two units and going permanently quiet. Source ownership persists on cooled crust. Persistent water contact quenches a fringe instead of permitting repeated hot/cold toggles. The actual hot core or buried vent is required. |
| Buried vents | Matching fire/poison geology still supplies an initially hidden vent, with a four-cell footprint limit and protected-placement checks. Initial activity is 40-80 Normal actions rather than the rejected 200-400. Lava vents can reheat their own cooled outlet; an extinguished ordinary pool does not invent a new vent. |
| Heat, ice and frost | Hot contact responds in stages after 5-7.5 Normal actions of sustained contact per stage, plus the five-action geology sampling cadence. Freezing water takes twice the base exposure and laying frost on a floor takes three times it. Newly frozen water is not a permanent cold source; frost is not a new moisture source. Ice melts into visibly weakening ice, then water, before water quenches lava. Occupied tiles remain protected. |
| Quartz | Restored the original one-in-1,500 eligible-cell chance rather than reducing it fourfold. Water and an existing natural vein remain prerequisites; dry granite cannot transmute into quartz. Whole new resource-bearing tiles remain exceptional and retain the eight-deposit limit and spent-mineral guard. This is deliberately not paced like a harmless recurring water edge. |
| Bridges | Sound chasm crossings remain stable without a damage cause. Moving water, acid and lava wear appropriate materials. Weakness is focused, and a protected sole-route bridge cannot monopolize all later weathering after reaching minimum integrity. Flood impacts retain force-based damage. Repair/new wood/new stone still cost 4/8/16 work actions and limited supplies. |
| Scheduling and saved games | All committed changes share the four-per-world-update limit. Warnings retain five normal-speed actions at every environment speed. External cancellation refunds genuinely new supply, but cancelling reheating cannot free an extra lava footprint unit. Save/load preserves the exact subsequent simulation. Cooled lava cells written by the rejected one-shot rule recover their missing ownership only within already spent capacity. No new save fields or version change. |

Action-driven systems reviewed and retained: flood plates release 1 + 8 + 16 path-reachable cells over two subsequent completed actions; elemental projections/brands change the impacted surface; melting ice has its explicit grounded-contact break chance and deep-water restrictions; earthquakes/destruction are bounded triggered area effects; mining and digging require actor actions; monster construction uses work and supplies. These systems already have visible causes and bounded operations and did not need a global timing reduction. Audible warnings, monster investigation/escape, critical routes, occupied cells, fixtures and authored protection remain in force.

Scientific mechanisms used as guidance, with deliberately accelerated game times: [NPS rockfall](https://www.nps.gov/yose/learn/nature/rockfall.htm) describes localized failure along fractures and accumulation of talus. [USGS pahoehoe flow](https://www.usgs.gov/news/volcano-watch-how-does-pahoehoe-flow) describes molten flow beneath a cooling crust, supporting continued source-fed activity rather than interpreting a cooled edge as an empty magma source. [USGS hydrothermal alteration](https://www.usgs.gov/news/alterations-go-hydrothermal-alteration-yellowstone) supports water-mediated mineral alteration. The fixed lava footprint, transient passable ledges and whole-tile quartz growth are gameplay abstractions; this is not a volume/elevation/chemistry simulation.

Production-engine evidence: 24 deterministic samples, Normal speed, 14 walls beside a chasm or a 3x3 liquid pool. Nearby changes count tiles changing within six grids and line of sight of a fixed observation point. This is an engine geometry measure, not a claim of interactive player visibility in darkness.

| Fixture | Rejected pass: nearby changes in actions 0-200 | Corrected: actions 0-200 | Corrected: actions 1000-1200 |
| --- | ---: | ---: | ---: |
| Chasm | 0.08 | 3.42 | 3.00 |
| Water | 2.12 | 7.12 | 6.58 |
| Lava | 0.92 | 3.50 | 3.25 |
| Acid | 0.54 | 7.12 | 6.58 |

In the first 200-action chasm window, each sample has two cracking transitions, one collapse and one rubble clearance (some changes are outside the observation area). Maximum standing rubble is one pile; its mean lifetime is 38.75 actions. In the late window, mean rubble lifetime is 40.83 actions. All liquid fixtures continue changing after 5,000 actions, while the lava fringe remains bounded. Tests now fail when early structural sequences never complete, a rubble pile lingers too long, liquid sources become inactive, or the footprint/change budget grows without bound.

Validation: incremental Windows build; `check_environment_balance.py` (127,595 assertions at this revision); `check_living_dungeon.py` (41,482 checks); current/legacy saves, corruption and exact 500-action lava continuation (1,575 checks). Existing bridge/flood/render checks from the first pass remain relevant to unchanged traversal and drawing, but the new pacing has not been interactively replayed on Android. Source configuration loads at process startup. The rejected metrics remain in ignored `scripts/output/environment-balance/rejected-slowdown-metrics.log` for comparison.

Implementation: `src/cave/cave-environment.c`, `src/cave/cave-environment.h`, `lib/edit/dungeon-dynamics.txt`, `src/ui/help.c`, and the environment behavior/save tests. Preserved the damaged-wall/quartz work and all unrelated changes already in the worktree.

## 2026-09-22: Visible counterplay, cooperation, and song responses
- Added completed-action tactical messages in `monster-ai.c`. Choice previews remain pure; a pending explanation is emitted only when the planned movement or deliberate retreat pause actually happens. Failed/skipped/displaced actions, unseen creatures and hallucination are suppressed. Cooldown is eight own actions, with at most one tactical message per player action. These presentation fields are explicitly unsaved.
- `melee-movement-tactics.c` tracks contributing score components for explanations. Messages distinguish breaking an Impale line, dispersing against sweeps, sidestepping repeated attacks, guarding, withdrawal cover, song responses and protected terrain. Ally descriptions require a relevant ally visible to the player; this visibility affects prose only, never the movement choice.
- Healthy melee creatures can screen a local ranged ally or cover a wounded fleeing ally. Guards preserve firing lanes, leave already covered posts to another healthy defender, and use the existing paid movement and hazard checks. Corrected the retreat-lane penalty to discourage blocking the route away from the player rather than the space between player and ally.
- Currently perceived Mastery/Lorien performances encourage ranged creatures to buy a few sound-flow steps while retaining a shot. Six sound steps bounds the extra spacing incentive; it is not an immunity radius. Elbereth has only a small extra positional preference and retains morale-driven fleeing. Aggressive Challenge compulsion is preserved.
- Contest/Lament targets use their own active pressure stacks to favor attack positions. Their duels have no escape radius or LOS break after targeting, so retreat receives no invented benefit. Song previews never inspect player song skill, Voice or selected duel target, consume RNG, or alter the effects of songs.
- Restarted the Luna Max song-research subagent after its earlier usage-limit failure; its completed rules audit informed the above semantics.
- Validation: AI/path/terrain suite (90,364 checks), combat selection, learning, real-engine movement/feedback/song integration and current/legacy save roundtrips pass. The engine fixture verifies a guard clears an orc archer's actual firing lane and an archer moves away from Lorien while retaining a shot. Standard Windows build passes. Interactive encounter balance remains untested.

## 2026-09-21: Monster AI review and protected terrain
- Fixed `src/melee/melee-movement-tactics.c`: deep-water displacement danger belongs to the player; a flying attacker must not zero it. Added a bounded preference for protected hazard squares that retain melee contact or a shot within preferred range. Fire resistance permits lava; poison resistance/flight permits pools; flight permits chasms, water and ice. Observed player resistance reduces the preference, and changed terrain is rescored each decision.
- Fixed `src/melee/melee-attack.c`: primary-blow knockback utility now passes the monster's coordinates to the displacement preview. Passing the player's position made its adjacency check fail, silently discarding all terrain payoff during blow selection.
- Repaired stale 6-point poison-dose expectations in AI and integration fixtures for the existing 8-point dose. No gameplay dose change. Tests cover safe/lethal crossing boundaries, action order, protected positioning, holding position, resistance evidence, mindless exclusion, and terrain changes.
- `scripts/check_monster_ai_integration.py` exercises real melee utility against a lava landing square and the complete get_move/make_move/process_move path: a protected creature steps onto useful lava footing, then resumes attacking.
- Player reactions already implemented: modest formation changes against Flanking; bounded hesitation/interception against Controlled Retreat/kiting; movement costs for witnessed Polearm/Opportunist/Zone; melee discounts against Riposte; dispersal against Impale/Whirlwind/Follow Through; hazard avoidance against witnessed knockback/exchange; repositioning against repeated attacks. Focus has only a small ranged-position incentive; song evidence increases mana-drain value. Stealth pursuit primarily uses the separate sight/sound/scent system. Stat/crafting abilities have no generic counter.
- Learning requires perception, alertness, and a thinking creature; ordinary creatures attenuate positional evidence. Memories expire after 8/20/40 player actions depending on type. Morgoth retains his separate tactical profile. Recall already exposes live memories in “Observations about you”; it does not explain which memory changed the chosen action.
- Suggested next work: expose the reason for actual selected counteractions with sparse feedback; add deliberate guard/withdrawal coordination beyond existing lane/formation scores; make witnessed song-specific responses respect each song's real range and effects. Existing warnings, rallies, quarrels, allies defending rivals, and leader mediation should be extended rather than duplicated.
- Validation: standard Windows build and real-engine AI integration pass; AI/path/terrain suite (90,309 checks), sensory suite (2,085), monster-ability suite (310), combat-selection suite (87), and learning suite pass. Interactive encounter balance remains untested.
...
## 2026-09-09: Shallow water in normal caves and Kheled-Zaram
- Water is persistent terrain (FEAT_WATER 84), not an inventory object. Existing IDs are unchanged. Vault glyph `_` places it; Kheled-Zaram now has 24 shallow-water tiles around the dry Durin artefact square instead of chasms. The wizard terrain editor also accepts `_`.
- `level-generation-water.c` builds compact connected lakes from interior seeds (up to one quarter of a cave's available floor, capped at 24 tiles), four-connected rivers between nearby cave anchors, and single-cave streams. Randomized bounded path searches validate whole river routes before carving. Existing tunnels remain available; paths protect authored rooms, vaults, doors, fixtures, items and special features. Only normal CAVEY partitions qualify; elemental caves, labyrinths and chasm areas do not.
- Grounded movement touching water costs 150% once, including exits, and applies -3 action Stealth. Stationary actions cost normally; flight ignores the movement penalty. Forced movement makes an immediate splash without charging a second player action. Successful single-tile Leaping uses the existing run-up/two-turn rules, skips water contact, and retains normal landing noise. A blocked wet landing pays contact once. Mouse travel prices water and wades; direct movement can leap.
- Water holds no scent. Scent aging continues, wet/airborne players do not stamp nearby land, and the land stencil cannot project through a stream or its diagonal corners. Old land tracks and ordinary sight/hearing remain. Floor drops stay visible and recoverable in shallow water.
- Four original `anim_water_surface_f0..3.png` files copied unchanged from C:/Assets/verdant-03-worlds-tileset-16x16/tiles/16x16. The shared SDL idle loop advances visible water every 320 ms without turns or RNG; remembered water freezes/dims. Forced cell repaint covers water creation/removal and viewport movement even when the base floor glyph is unchanged. Existing anim_*.png build staging includes these assets.
- Monster energy is signed so water debt cannot wrap. The record reader accepts legacy byte energy and uses signed values from 0.9.8.2 onward. Concurrent disease work introduced 0.9.8.3 and added its own water exposure hooks; those changes and all other concurrent edits were preserved.
- Validation: all-core standard Windows build passed; scripts/check_water.py passed actual movement/Leaping and monster movement, scent barriers, 100 actual CA cave layouts, excluded partition/feature protection, complete old/new monster record alignment, Kheled-Zaram building in 16 rotations/reflections, supplied-frame SDL rendering, fog/idle behavior, pan/removal in both tile widths, and the existing torch/overlay regression suite. The suite wraps unrelated quest/UI feedback, forge contents and artefact creation; it validates water geometry, not the full encounters or physical inputs.
- Preview captures: scripts/output/water-check/cave-water.png and kheled-zaram.png. Detailed test/build logs are scripts/output/water-check.log and water-build.log. Standard deployment executable, terrain/vault templates and four water frames refreshed and hash-verified. Manual gameplay balance, controller/touch hardware and Android deployment are not yet replayed.

## 2026-09-09: Molten lava
- Added FEAT_LAVA (85), preserving existing IDs and save layout. Fire big caves get connected pools and bent rivers, retaining existing dry routes and protecting stairs, room anchors, vaults, objects and fixtures. Vault glyph backtick adds lava; Flame Pits (420) replaces ten chasms, Gothmog's hall (402) gains two pools. Wizard terrain editing also accepts backtick.
- Ground contact without net fire resistance kills immediately through the ordinary player death route. With 1/2/3 net resistance stacks, entry and subsequent occupied actions cause 40/30/24 HP damage (60 raw, existing resistance curve, no armour). Equipment, temporary resistance and the fire-cave penalty count. Flying monsters take 40 heat damage per action; fire-resistant monsters take none; other monsters die. Forced movement/terrain creation use the same exposure route and death guards prevent further attacks on removed actors.
- Leaping with a run-up crosses one lava tile to a known bank; flight adds one effective resistance stack and takes heat damage. Midpoint entry and landing do not double charge. A newly blocked landing applies ground exposure. Automatic travel avoids lava; direct movement warns with destination-adjusted damage. Help, interaction and inspection describe the rules.
- Lava supplies a wall-occluded radius-two light field, brightest contribution 3, recomputed from terrain without stacking pool tiles. Four original animation frames came from adjacent Verdant 03 Worlds (the requested Depths directory contains no lava); source license included. Shared water scheduler renders liquid beneath transparent actors, freezes remembered tiles, and preserves RNG/action timing.
- Validation: all-core standard Windows build passed. check_lava.py passed real movement, cancellation, lethal/resisted contact, potion tiers, entry/stationary/free-action timing, successful/blocked jumps, knockback, path exclusion, transformation, light occlusion, monster forced entry, flight/immunity and skipped/sleeping turns. Prompt responses and death persistence are isolated; water/fixture regressions pass in the same harness. check_lava_generation.py passed 200 seeded maps and both vault dry-route checks; check_lava_animation.py passed exact source pixels, covered tiles, fog, pan/removal and idle state checks. No manual balance/controller/touch/Android replay.
- Standard executable, terrain/vault templates and four lava frames were staged and hash-verified. Deployed binary with dummy SDL drivers and isolated user/data/save/meta paths reached initial_menu under debugger after runtime template parsing; no ERROR/FATAL records in its game log. Evidence: scripts/output/lava-build.log, lava-check.log, lava-startup-check/, lava-animation-check/lava-scene.png (constructed renderer preview).

## 2026-09-11: Monster sound pack coverage
...
## 2026-09-14: Tutorial mode selector and settings placement
...
## 2026-09-17: Repository-wide code review and fixes
- Reviewed first-party persistence/inventory/scoring/metaruns, combat/AI/status effects, generation/template parsers/build/Android resource routes/Tile Forge, SDL rendering/input/UI/tutorials, and shared formatting. Three Astra high reviewers covered independent subsystems. Existing worktree edits and untracked Help export were preserved; renderer work also changed concurrently and was validated in the final build/tests. This was a broad source review and regression pass, not an exhaustive proof of every path or a review of vendored dependencies.
- Save notes: `src/fs/load-notes-inventory.c` detects EOF instead of looping forever and only accepts an exact delimiter record; `src/fs/save-notes-inventory.c` streams records without the old 100-byte stack buffer, preserving long and partial final lines. Existing normal bytes and save version stay unchanged.
- Legacy Quiver migration removes the old equipped slot before calculating capacity and retains all unaccepted arrows in Pack. Regression covers every count 1..99, mixed representations, adjacent Pack compaction and a full Pack with exact conservation.
- Combat: auxiliary attack refusal no longer refunds movement or a later Rage/Whirlwind/Impale attack. Multi-target cancellation is resolved by the requested action. Monster movement interrupted by Zone of Control/Knock Back no longer collects or crushes items at its unreached destination; completed movement still does.
- Quest parsing initializes default depth bounds at record creation, preserving `E:DEPTH_RANGE` when `P:` follows. Shipped Varda/Orome bounds and interpolated odds are checked against the real parser/calculator. The updated template also invalidates old caches through existing refresh mechanisms.
- Both release packaging scripts include `lib/help`. Added the missing deep-water bridge/deep-water/flooding-trap catalogue entries and a Normal deep-water warning; guided opening movement excludes deep water. Canonical tutorial reference regenerated; strict coverage now passes at 536 lessons.
- Map rendering rejects off-map coordinates before cave-array access and preserves darkness for special-feature underlays hidden by rage/labyrinth. Both map lookup APIs have regressions. Shared formatting honors zero-capacity buffers and bounds numeric conversions before copying into the caller buffer.
- Repaired stale controller expectations to retain current overlapping-press/released-tap semantics; restored the tutorial FOV fixture's object-index array. Current monster save fixtures now use VERSION_EXTRA, while genuine legacy records omit subsequent format additions. Corrected an oversized memmove in the scent compatibility fixture.
- Final all-core standard and portable builds pass. Final focused scripts pass: check_save_notes, check_legacy_quiver_load, check_combat_action_energy, check_format_bounds, check_quest_formula_bounds, check_pickup_destination_preview, check_help_controller_runtime, check_gameplay_tutorial_integration, check_material_edges, and strict check_tutorial_catalogue. Other passing coverage includes 90,286 AI checks plus flow integration, 74 combat checks, 76 sensing checks, 310 monster ability checks, scent persistence, water/lava/disease/poison mechanics, 600 generated terrain maps, four complete depth-generation fixtures, 115 theme-parser cases, template versions, 22 Tile Forge tests, startup input, context popups, tutorial rendering/archive/visibility/world/start/upgrade and FOV.
- Final portable executable launched with dummy SDL drivers and isolated runtime data under `scripts/output/review-startup`, reaching `initial_menu` after parsing runtime data; no ERROR/FATAL entries. Logs are `scripts/output/review-build-{standard,portable}.log`, `review-final-*.log`, and `review-startup/startup.log`. No actual player save/config was loaded. Manual gameplay, physical controller/touch, Android/iOS device testing, release packaging execution and publication were not performed. No commit or release was created.

## 2026-09-20: Vampire sound tail trim
- Recut all 20 assigned AlesiaDavina Vampire OGGs at decoded signal breakpoints using page-level Vorbis EOS cuts, preserving the existing Vorbis packets without re-encoding. Final durations are approximately 0.56-1.66 seconds for attacks/hits/hisses, 1.25 seconds for bored, and 2.25-2.67 seconds for deaths.
- Validation: `check_monster_sounds.ps1` passed assignment consistency, SDL_mixer playback/cache checks, and all OGG decode batches. No deployment or audible listening was performed.

## 2026-09-20: Branch-wide C review (current pass)
- Scope: first-party C on branch 0.9.8, including older code; tooling and vendored dependencies excluded from review. GCC -fanalyzer compiled all 338 first-party C translation units. Terrain placement required reduced analyzer recursion/state limits after the default analysis exited without diagnostics. Manually traced reported warnings and reviewed persistence, scoring/metaruns, inventory, generation, combat, terrain, input and rendering paths; this is not proof that every runtime path is defect-free.
- Artefact databases: score_artefact.c and metarun-artefact-memory.c now preserve invalid/incompatible existing databases, validate version/count/file length, and avoid truncating an existing file when opening it for update fails. Header updates and close failures propagate to callers. Regression covers create/reopen/append/update plus byte-for-byte preservation of future versions, invalid magic, short headers, partial records and count mismatches.
- Kinslayer: accepts both normal space-padded and synthetic zero-padded score IDs. Dead/escaped heroes remain ineligible, and existing live entries are updated rather than duplicated. Failed writes no longer report a successful kill. Corrected inverted SDL3 close-result tests. In-memory production-code regression covers both encodings and failed writes.
- Truce: retain the actual alert witness for the noise origin instead of using the final scanned monster slot. Regression covers a dead/asleep/out-of-sight final slot, range invalidation, no witnesses and an obvious break.
- Revalidated earlier inventory exchange and vault-token fixes now present in HEAD. Preserved the user's save-compatibility commits and character.txt edits.
- Regression fixture maintenance only: preserve ice-map boundary wall flags, initialize the current flood-kind reader flag, supply the required eight-byte save tail, account for all three flood blocks, and read newly written Utumno dungeon records at VERSION_EXTRA. Scent harness uses the SDL DLLs matching its build. No production save-format change was introduced by this pass.
- Final standard build: build-incremental.ps1 -Jobs 4 PASS (scripts/output/review-final-build.log). New regressions check_artefact_databases, check_kinslayer and check_truce PASS. Save checks check_flood_surface_save (legacy v14/v15 and v16, corruption/truncation and checksums), check_monster_scent_save (including legacy v5/v6), check_utumno_state, check_save_notes and check_legacy_quiver_load PASS. check_ice, check_water_flood, check_pickup_destination_preview, check_combat_action_energy, check_utumno_generation and check_format_bounds PASS.
- Earlier in this review, production checks for water/lava/submerged items/melting ice/river acid tiles, terrain generation, Utumno travel, disease, poison, bridges, AI/combat/senses/abilities/sounds, tutorial integration/upgrade and birth defaults also passed. No actual player save/config was opened; no manual gameplay, physical controller/touch, mobile-device run or new portable build is claimed for this pass. Changes remain uncommitted.

## 2026-09-20 Living dungeon and environmental monster goals
- Added cave-environment.c: bounded reservoirs, rise/recession, wall wear, finite quartz exposure, thermal diffusion across partitions, floor-ice substrate, dormant elemental vents, warnings, historical bridge damage and finite multi-action construction. World clock and independent RNG persist; mandatory reachable anchors are preserved.
- Added cave-events.c: obstacle-aware physical events and ambient masks, directional audible message-log notices, warning disturbance, movement-only masking preserving visual detection. Earthquakes/destruction now announce their physical origin rather than player coordinates.
- Added monster-world.c: independent environmental observations/goals, hazard evacuation, bounded investigation, local repair/construction claims and supplies. Explicit BRIDGE_BUILDER on Orc soldier, Orc warrior and Easterling warrior; jobs survive actor compaction and never overwrite player sensory clues.
- Save extra 17 appends typed environmental RLE, source budgets, event ring and per-monster world tasks. Legacy saves seed conservative environment state. Remembered feature, underlay and deck material prevent unseen terrain changes leaking through rendering.
- New tuning: lib/edit/dungeon-dynamics.txt. Updated Help, learned monster lore, bridge damage rendering and terrain inspection. Untracked docs/help-pages preserved.
- Validation: standard parallel incremental build; check_living_dungeon.py (38,285 checks); check_living_dungeon_save.py (607 checks, current/legacy/corrupt/truncated); sound tests22 and sensing518; bridge movement/save/SDL/damaged-deck tests; ice-water rendered transitions and viewed previews; scoped diff check.
- Staged standard exe plus monster.txt/dungeon-dynamics.txt with matching SHA256. Launched sil-more-windows-sdl3/sil-more.exe with temporary data/save/user/apex paths, reached main menu, then stopped only the smoke-test process. No live gameplay/balance or physical-device claim.
- Added default-off Interface > Log all dungeon events in messages {debug}. App JSON key interface.show_dungeon_events reuses retired auto_more slot 71; legacy migration still clears the old bit and app settings take precedence. Logs every valid physical event at emission with source coordinates, turn and volume, regardless of hearing/cooldowns/ring replacement; normal hearing and AI are unchanged. Validation: sound checks, 38,295 ecology/settings checks including JSON on/off and old-key defaults, standard parallel build and diff check. Moved the toggle out of Debugging Options at the user's request; full and compact labels retain {debug}.
- Added Ctrl+Y > Map and Travel > Teleport to last dungeon event (h). Uses the latest valid current-level event regardless of logging/hearing; expired records survive existing v17 saves without becoming audible again. Chooses the closest empty floor within sight range of the event, excludes pending hazards, and uses normal teleport movement/redraw. No event or no safe landing produces a message without moving. Validation: standard build; 93 sound checks; 38,308 ecology/settings/wizard checks through the actual menu dispatcher and teleport; 610 persistence checks including expired-event history and legacy reset.
- Added Gameplay > Living Dungeon > Environmental effects speed: Slow retains the original cadence, Normal defaults to 2x, Fast to 4x. Initial reservoir/vent delays, source intervals and geology pulses scale; changing the setting rescales remaining source waits without advancing RNG/time. Warnings, monster action speeds/work and change budgets are unchanged. App-only gameplay.environmentSpeed JSON (0/1/2) persists the setting, with old/missing/invalid configs defaulting to Normal; no dungeon save layout change. Uses retired UI slot 89 and a dedicated option byte. Validation: parallel standard build; 38,411 behavior/UI-picker/default/persistence checks; 610 dungeon save checks.

## 2026-09-21: Monster groups, local quarrels, and fighting noise
- Added `src/monster/monster-social.{c,h}`. Race-derived groups start mutually neutral; explicit IDs 8..15 allow mixed-race bands. `monster_social_set_group()` changes membership, `monster_group_set_relation()` sets mutual neutral/hostile diplomacy, and `monster_social_start_feud()` starts a personal two-monster quarrel (including within one group). Setting groups neutral ends their personal feuds too. Diplomacy is local to the current dungeon level.
- Idle orcs have one 1/500 chance per eligible action to challenge an adjacent orc/man/elf. The challenge consumes an action; fighting uses physical melee, actual HP/armour, retaliation, and short visible pursuit. Feuds end after 20 actions without seeing the rival and impose an 80-action quarrel cooldown. Uniques, peaceful NPCs, and both types of thrall keep their dedicated behaviour; player tracking, fleeing, incapacitation, and the throne-room truce take priority. Territorial creatures do not pursue rivals or regenerate their hoard on death.
- Ambient kills release loot without player XP, kill counts, or quest consequences. Spell effects and player-specific melee abilities are outside this initial monster-versus-monster combat implementation.
- Challenges/attacks emit CAVE_EVENT_FIGHT (volume 12/24), alongside existing melee/damage/death audio. Real propagated noise reduces hearing locally (up to 8 at a blow's origin), including stationary player actions; sight and detection of sufficiently loud noises still work. Walls block, doors attenuate, and sound fades over 60 world ticks. Gameplay masking works with audio muted. Listen can also detect the fighting creature's increased noise.
- VERSION_EXTRA 19 saves group diplomacy, membership, rivals, expiry, and cooldown after the environment block. Older saves load neutral defaults. Deletion/compaction repairs rival indices; leaving a level resets diplomacy. Readers reject invalid, asymmetric, and truncated social records.
- Validation: `build-cmake.bat standard` built and refreshed the standard deployment. `python scripts/check_monster_social.py` passes full-engine combat/retaliation, orc-human quarrels, action scheduling, loot/quest accounting, territorial drops, compaction/slot reuse, current/legacy save roundtrips, and corrupt-tail checks. `check_monster_thralls.ps1` (3394), `check_monster_senses.ps1` (2085), `check_cave_events.ps1` (105), and `check_monster_ai_combat.ps1` (74) pass. Scoped diff whitespace checks pass. No interactive audio/UI smoke test was performed.

## 2026-09-21: Dispute escalation, witnesses, and relationship-aware cooperation
- `monster_social_allies()` now governs morale/fear, warnings and scent sharing, formation/shadow support, Rally, control follow-up, and breath collateral. Personal/group hostility overrides kinship; distinct explicit bands do not cooperate merely because they are neutral. Actual wandering companions, matching inferred groups, and matching custom bands can cooperate; OTHER does not unite unrelated creatures.
- Added paid challenge and fighting phases. Unwary fighters can yield at half HP or when substantially outmatched, independently of the player-facing stance; ordinary disputes also expire after 16 fighting decisions. Yielding ends the reciprocal feud and its helpers, with safe local withdrawal for up to eight actions and the existing 80-action cooldown. Underlying group hostility remains intact.
- Witnesses require sight of both combatants within six squares. Up to two helpers per side can defend an ally in a personal dispute; wounded/outmatched witnesses withdraw, and a capable captain trusted by both parties can stop the quarrel. Hostile group clashes also expose combatants to witness reactions. No witnesses acquire identities from sound alone; observation uses a bounded local grid scan.
- Stale player tracks no longer veto ongoing social combat. A visible player within three squares or closer than the rival, or recent direct player aggression (including absorbed hits), takes priority. Incapacitation, Smite recovery, imminent hazards, protected NPCs/uniques, and truce retain their gates.
- Visible monster recall describes confrontation, quarrel, assistance, or withdrawal. VERSION_EXTRA 20 adds social state/timer, witness references, and recent player threat. Version 19 feuds load as bounded fights; older saves retain neutral defaults. Save validation and deletion/compaction cover all new references.
- Validation: standard Windows build/deployment; full-engine social suite including escalation, injury retreat, attention, witnesses/captains/helper caps, loot/quest accounting, save corruption/legacy compatibility, and 100 scheduled multi-band actions with diplomacy changes and repeated saves. Existing executable suites passed: AI 90,292 checks; combat 85; senses 2,085; thralls 3,394; learning passed. Fixtures link the real social module and add hostile formation/Rally/collateral checks. No interactive gameplay/UI smoke test performed.
## 2026-09-21: River/lava cadence and poisonous-acid dose tuning
- Halved the Living Dungeon water and lava source intervals in both the built-in fallback and `lib/edit/dungeon-dynamics.txt` (`water_step` 400->200, `lava_step` 700->350). Acid source cadence remains 600; Normal/ Fast environment settings still apply their existing 2x/4x multipliers.
- Raised `POISON_TERRAIN_DOSE` 6->8. The existing resistance, Protection, delayed poison tick, flying/leaping avoidance and acid-item paths remain unchanged; Help, interaction text, tutorial reference and poison harness expectations now describe 8 raw stacks.
- Validation: incremental standard build, poison terrain, water, lava, river/acid rendering, and 610-check Living Dungeon save harness pass. The full ecology harness reaches the updated cadence checks but retains a pre-existing actor test mismatch: it expects a stone chasm bridge after 8 work actions although production requires 16; observed state is `work=8`, `supplies=16`, floor unchanged.

## 2026-09-24: Global positional sound and soft range boundaries
- Central tuning: `src/sound-spatial.h`. All located effects share a squared falloff: distance 0/1 = full category gain; otherwise `((radius + 1 - acoustic_distance) / radius)^2`, with zero beyond the radius. The last audible tile is included in each radius below. These are amplitude/gain percentages, not perceived-loudness percentages.
- Chosen radii: wall torches 3, braziers 4; water/acid, lava, forge fire and bridge-work loops 6; ordinary interactions, drops, footsteps and trap mechanisms 6; monster idle/flee 11; combat/armour impacts, digging, horns, door bashes, alarms, collapses and floods 21. Idle/combat keep their prior broad footprint plus one quieter tile; short ambience stays local.
- For radius 6, relative gain at distances 0/1, 2, 3, 4, 5, 6, 7 is 100%, 69.4%, 44.4%, 25%, 11.1%, 2.8%, 0%. Environmental loops retain their 50% base scale, so the outer tile is 1.39% of the category slider, compared with the old outer tile's 18.75%. Radius 11 reaches 40.5% at d5, 3.3% at d10, 0.83% at d11; radius 21 reaches 65.5% at d5, 32.7% at d10, 11.1% at d15, 0.91% at d20, 0.23% at d21.
- `cave_audio_distance()` caches a listener-centred acoustic field. Walls block traversal, closed doors add four extra path tiles, and diagonal closed corners do not leak. A door's own sound does not pay absorption for passing through itself; wall fixtures emit into exposed neighbours. This field is separate from AI hearing, event volumes, and saved state.
- Six independent ambience tracks mix with the existing 16 SFX voices. Multiple sources of one ambience type contribute their strongest audible source, preventing a whole lake or many torches from multiplying volume. A 16 ms SDL timer completes linear 160 ms gain transitions even while input waits; reversing a fade keeps the loop playing. Active SFX stay at the event's original grid, fading when the listener moves. Delayed cues preserve their origin and refresh pending gain on the game thread; timers never read dungeon/player state. Music and feedback outside dungeon play are listener-local.
- Added positional `sound_at`, `sound_delayed_at`, `message_at`, and `message_format_at`; typed messages retain history/colour. Migrated monster sounds including forced trap idle cues, monster death/flee, chest/door/trap events, teleport departures, forge destruction, dropped/scattered objects, and delayed melee impacts. Added matching positional audio for monster door/dig actions, spell-driven door actions, rewired traps, and projectile impacts using existing recordings. Sleeping-idle gates, probability, explicit per-monster assignments, and all existing category controls remain in effect.
- Validation: Windows standard parallel build; 165-monster/272-assignment audit and 461 OGG decodes; real SDL dummy-device playback tests for near/far gain, cutoffs, forced idle, water footsteps, moving listeners, delayed origins/cancellation, six-loop mixing, fade reversal/completion, mutes and cache recreation; 126 world-sound checks; 2,085 monster-sensing checks. Fixed test-script bridge linkage for the current terrain module. Built executable starts with current source assets, opens the audio device and exits cleanly without log warnings/errors. No in-game listening/audition was performed; these distances are an implemented balance starting point, not a claim of perceptual optimality.
- Final staging: copied/hash-verified `build-standard/sil-more.exe` to `sil-more-windows-sdl3/sil-more.exe`; all staged edit/config/non-WAV sound files matched current sources. That exact staged executable started, opened SDL audio, and closed normally with no warning/error log entries. No files in `C:\games` were changed.

## 2026-09-24: Subtle Stealth scaling for player footsteps
- Player `MSG_WALK` source gain is `1 - 0.2 * S / (20 + abs(S))`, where S is `skill_use[S_STL]` at emission. Effective skill already includes equipment, armour, rage, songs and stealth mode; no duplicate stealth-mode bonus or use of the action's mutable `stealth_score` (which can include flanking noise). Gain is 110% at S=-20, 100% at 0, 90% at +20, 84% at +80, asymptotically bounded to 80%-120%. Float conversion precedes the absolute value to handle INT_MIN safely.
- `sound_origin.source_gain` captures this modifier and combines it with category/master volume and spatial falloff for normal and water footsteps, active clips and delayed playback. Subsequent skill changes do not rewrite an emitted step; remote footsteps, landings, other effects and AI noise/detection are unaffected.
- Extended the real SDL playback harness to cover both footstep banks, positive/negative/extreme scores, category multiplication, movement after skill changes, delayed snapshot retention, remote sounds, leap suppression and muting. No perceptual listening claim; this is deliberately gentle gain modulation.
- Validation complete: standard parallel build and full monster-sound suite (including new Stealth checks) passed; standard staged executable hash matches build, starts and exits with code 0 and no log warnings/errors. Scoped whitespace check passed. In-game listening remains unverified.

## 2026-09-24: Recalibrate footstep Stealth around reachable values
- Supersedes the previous denominator 20: gain is now `1 - 0.2 * S / (5 + abs(S))`. +5 gives 90%, +10 gives 86.7%, +15 gives 85%, +20 gives 84%; -5 gives 110%, -10 gives 113.3%. The 80%-120% bounds remain, and only the response scale changed. +80 is not a useful ordinary-play calibration target.
- Evidence: live `calc_bonuses()` sums base + DEX + equipment + miscellaneous modifiers; stealth mode adds 5, armour costs 1 per 10 lb, affinity is clamped to -2..2, and ordinary song penalties are 4..12. Training costs `50*n*(n+1)` XP from rank zero: rank 10 costs 5,500 and 20 costs 21,000. This establishes the low-tens scale without claiming a hard cap on effective Stealth.
- Read-only check of 12 existing character dumps (May-September 2026) found effective Stealth [-1,2,2,2,2,4,4,5,5,6,7,20]; ten were 2..7. These are historical examples, mostly early-run/base-zero characters, not a measured population distribution or current maximum. The old curve only reduced volume 4% at +5; the revised curve reduces it 10%, matching a meaningful built-in +5 increment while retaining subtle modulation.
- Updated playback regression examples to +/-5 and arithmetic checks to +5/+10/+20, retaining extreme-integer robustness, material routing, category scaling, source snapshots, delay and distance checks.
- Validation: standard parallel build, full sound regression suite and scoped whitespace check pass. Standard staged executable hash matches the build. Launch log confirms audio initialization and menu entry, followed by a clean menu exit (code 0) before the smoke-test delay ended; no warning/error entries. No sustained gameplay or listening validation is claimed.

## 2026-09-24: Android Auto UI false controller detection
- `sdl_android_has_controller_device()` now requires a successfully opened SDL gamepad before accepting Android source flags. The JNI scan also excludes `uinput-*` fingerprint/sensor devices, matching SDL's exclusion, so a sensor plus a tablet keyboard cannot jointly enable controller UI. Existing alphabetic-keyboard and Android virtual-device exclusions remain.
- Removed obsolete raw-controller-with-zero-SDL-gamepads diagnostics. Startup and controller hotplug use the same corrected detector. No persisted settings or defaults were changed.
- `scripts/check_android_input_detection.py` compiles the production detector, UI gates and hotplug handler with fake JNI metadata and SDL open/close results. All 24 cases pass: clean phone, three sensor names, keyboard/sensor combinations, genuine controller, failed open, late connect/disconnect, metadata failure, explicit modes. The original detector fails this regression. Physical device behavior is not simulated by this harness.
- Validation: Windows incremental parallel build, desktop startup/dialog regression and controller contract pass; signed arm64 sideload Release APK built successfully at `android/app/build/outputs/apk/sideload/release/app-sideload-release.apk` (0.9.8 / 90821, `com.silmore.myapp.sideload`). No ADB device was connected; Redmi/Retroid physical replay remains unverified.

## 2026-09-24: Correct diagonal sound paths around wall fixtures
- Android report: one brazier is silent while other torches and normal game effects work. The screenshot shows a wall-mounted fixture exposed diagonally to the room. The playback wall-source query incorrectly required both cardinal flank cells to be open before accepting an exposed diagonal neighbour, making the fixture's source completely unreachable. Recreated the matching local arrangement in all four orientations; the regression failed before the fix.
- Wall-mounted sounds now emit into all eight open neighbouring cells. This is emission from the wall surface, not traversal through the backing wall. All playback propagation (positioned effects, flowing/still liquid, lava, forge and bridge loops) now allows a diagonal around one exposed corner. Two blocked flanks still prevent through-corner transmission between floor cells, and entirely enclosed fixtures remain silent. Rubble passability/cost is unchanged; replacing the cardinal rubble with solid wall in the fixture test still preserves the exposed diagonal route.
- Existing gameplay event fields and ambient hearing masks retain their prior corner rule. Production tests cover all fixture types adjacent diagonally, four fixture orientations, cache invalidation, enclosing walls, open single corners, closed-door attenuation, water-loop consistency and unchanged gameplay hearing. World-sound suite: 179 checks; monster-sensing suite: 2,085 checks. Standard Windows parallel build passes. No Android device is connected via ADB, so physical replay/listening cannot be verified here.
- Android delivery: `build-android-apk.ps1 -Config Release` succeeded for `com.silmore.myapp.sideload` / arm64-v8a. Verified the APK's native library matches the current stripped build output and its Torch Loop.ogg matches the source asset. Output: `android/app/build/outputs/apk/sideload/release/app-sideload-release.apk`. No device install performed.

## 2026-09-25: Separate quartz deposits from damaged walls
- Added appended features 104 damaged wall and 105 cracked quartz; version extra 22. Shared rock/wall predicates cover movement, monsters, terrain access, minimap and SDL rendering. Old architectural quartz is conservatively migrated on loading pre-22 maps; natural cave/chasm quartz remains, while old authored vault provenance cannot be recovered.
- Digging retains tool/Strength thresholds (granite 3, quartz/damage 2, rubble 1). Eligible strikes always progress; opposed Strength margin >=5 skips to rubble. Blasting classifies once, tests Will once (granite 14, quartz/damage 12), and uses the same stages. Earthquakes create damaged stone instead of minerals.
- `cave-rock.c` centralizes quartz rewards on actual destruction: one depth-generated gem (Light fallback if the pool is empty) and a separate metal roll. Mithril begins at 600 ft; star iron only in tagged chasm partitions, from 500 ft. Cracking and clearing rubble do not pay again. Unseen mineral drops do not identify gems.
- Natural quartz scatter requires cave/big-cave/chasm placement plus exposed natural ground, excludes authored construction, and falls from 30% to 5% per pass. Generic streamers now place damage and preserve vaults. Vault '/' is damage; only Slave mine and both Glittering Caves retain explicit '%' deposits (32 cells total).
- Environmental erosion exposes damage before collapse, while finite quartz growth has a separate mineral event and excludes ruins/construction. Preserved concurrent environmental balance changes in cave-environment.c and dungeon-dynamics.txt.
- Damage retains existing style Y/default E art. Quartz derives a milky-white light/dark overlay from the same alpha mask into reserved runtime atlas row 60; renderer reset uses the same loader. Preview: scripts/output/quartz-walls/quartz-preview.png (intact/damaged/quartz/cracked, lit/unlit).
- Validation: standard parallel build; focused quartz mechanics, actual gem generator/fallback, save/load v22 and migration v21, generation and SDL tests; cave-floor/idle-animation suite; generated depths 3/10/16 seed 1301; terrain fresh/cache/error/deployed checks. The older living-dungeon-save harness failed an extension-size assumption; the new focused fixture verifies current and v21 terrain roundtrips. Full tutorial audit still lacks pre-existing terrain.103; new lessons 104/105 and updated 51 are authored without changing unrelated Sprinting edits.
- Staged standard executable and changed terrain/object/vault/style/help templates. Executable SHA256 matches build output. No interactive gameplay or Android-device replay performed.

## 2026-09-25: Commander and soldier combat positioning
- Added one squad planning pass before monster actions. Rally/escort leaders assign up to eight eligible nearby allies separate melee/ranged destinations; stronger leaders take priority, firing positions are reserved first, and soldiers retain existing hazard/reaction/morale checks. Safe ranged posts are held instead of abandoned by ordinary advance.
- Orders require current commander sight and local allied contact; truce, Disguise, confusion, stun, fear and recovery disable participation. Runtime-only `monster_type.squad` is reset on load/spawn, cleared on leader deletion and remapped during compaction; no save-format change. Visible order-following movement uses existing tactical feedback throttling.
- Implementation: `melee-movement-tactics.c`, movement/scheduler hooks, AI reset/feedback, social remapping and `types.h`; details in `docs/monster-ai-plan.md`. Existing unrelated dirty work is preserved.
- Validation: 90,441 focused AI checks, 87 combat checks, learning suite, 2,085 sensing checks, six production Fankil squad scheduler rounds including leader death/index reuse, legacy monster-record versions 0.9.8.2-7 roundtrip, and standard parallel build. Refreshed `sil-more-windows-sdl3/sil-more.exe` with matching SHA256 to the build output. Interactive encounter balance and Android gameplay remain untested.

### Leadership grades and ordinary-group fallback
- Replaced the initial Rally/Escort gate with authored `L:grade:doctrine:kin[:extra kin]` monster profiles. 105 profiles, including 50 non-unique leader-capable types; ORDINARY/LEADER/COMMANDER grades determine capability and succession. NONE permits following only; unprofiled and mindless monsters do not join squads. No combat-stat or spawn-flag changes.
- Added strict template parsing and race metadata. Command kin prevent shared wandering bands from granting unrelated authority. Only explicit tactical commanders gain extra kin: Gothmog/Turkano command orcs/trolls alongside raukar, Fankil commands raukar alongside Men. Diplomacy and conflicting custom allegiances still win. PACK wolves/spiders pursue/surround without military guard/assembly orders.
- Viable incumbents beat same-grade newcomers; higher grades can take over. Existing members are retained first. Experienced leaders assign a flanker and guard/withdrawal cover; commanders can assemble widely spread approaching melee for two player turns or regroup a half-wounded squad for three. Persistent triggers cannot renew the budget. Assembly excludes ranged troops and the commander and stops before close engagement.
- Tests caught and fixed ordinary melee preference overpowering regroup orders, and assembly incorrectly treating rear archers as lagging melee. Production fixtures now execute ordinary-orc coordination, champion takeover/death, and actual get/make/process_move retreat alongside the six-round Fankil encounter.
- Validation: 90,482 focused AI checks, 87 combat checks, learning suite, old monster-record roundtrips, template version audit, real leadership parser/invalid-input tests, raw-cache load and old-record-size regeneration, and parallel standard build. Interactive balance and Android gameplay remain untested.
- Staged and SHA256-verified the updated standard executable and `lib/edit/monster.txt`; existing raw caches regenerate automatically without deleting player data.


## 2026-09-26: Catastrophe pacing and cold through structures
- Removed step-based eightfold acceleration and recursive same-substep spread. Liquid fronts advance one or two snapshot layers per action, averaging 1 + anger/6. Other materials transmit in one or two actions with deterministic saved arrival phases; solid terrain adds one action for cold. Immediate activation, completed-action guards and existing event/milestone rules remain.
- Fixed source budgets per action: water/acid 32+8a, lava 24+6a, chasm 16+4a, cold field 128+32a. Water shares supply between shallow spread and deepening (minimum 6-floor(a/2) subsequent actions); narrow/blocked fronts can use remaining supply to deepen. Acid/lava/chasm wall wear 5+2a / 15+3a / 20+4a; each structure receives contact once per action.
- Cold crosses intact interior walls, closed/open doors, traps, bridges, forges and protected support. Structures retain terrain IDs; floor and sufficiently cooled water become ice. Exposure follows the reached cold field, including decks, with one cold pulse/action. Existing hot-fringe melting/quenching remains. No new save fields: .23 cell ages and saved RNG preserve continuation.
- Focused tests: 2,462 passed, including 12 preserved structure types, finite shared water supply, one contact across two liquid substeps, exposure on preserved decks and exact serialized 12-action continuation for all five materials. Windows parallel build plus environment 41,482, persistence 1,575, flood, terrain transitions and combat action regressions passed.
- Final pacing dataset: 356 production-controller runs x 200 actions, 15 generated maps at 100/250/500/750/900 ft plus open rooms/corridors and matched-source comparisons. At anger 6, 50x50 room t90 water/acid/lava/ice/chasm = 38/30/41/24/59 actions (previous 9/9/14/14/24). Water/acid travel 100 corridor tiles in 100/75/60/50 actions at anger 0/2/4/6; others 199/164/126/100. Natural-source water median coverage at 40/80/120/200 actions: 18.6/42.5/68.8/91.1 percent. Natural ice evidence is one map; matched-source ice covers five layouts.
- Reproduce with scripts/analyze_catastrophe_speed.py; report, CSVs, plots and source/object hashes in scripts/output/catastrophe-speed. Baseline preserved under before-pacing. Simulations use a stationary protected observer, no moving monsters or ambient geology; no human escape or visual gameplay validation claimed. Standard staged executable copied and SHA256 matched to the final build.
- Independent read-only Luna Max review found no further actionable regressions after the bridge-contact fix. Added explicit coverage for severed source connections: detached water/acid/lava/chasm fronts stop advancing (and water stops deepening there), then resume after reconnection; cold continues through the intact barrier.
