 ...
 * the caller that side-by-side layout is impossible, so the pane retains its
 * full readable width. */
    ...
    status_depth_pane_layout* out)
{
    ...
    *out = (status_depth_pane_layout){ 0 };
    ...
    /* Quick Access keeps its configured geometry.  Status & Depth in another
     * slot on the same screen edge consumes only the side span before it and
     * wraps into additional rows; an exact shared slot uses normal stacking. */
