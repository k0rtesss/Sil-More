...
/*
 * Transitional public API for the cave split.
 *
 * Lane C owns this surface so later cave extractions can stop publishing new
 * cave declarations through src/externs.h.
 */
...
void styles_set_vault_rule(int depth, const int* sidx, const int* weight,
    int count);
void styles_apply_vault_default_for_depth(int depth);
void styles_partition_rules_clear(void);
void styles_add_partition_rule(int depth, int kind, const int* sidx,
    const int* weight, int count);
