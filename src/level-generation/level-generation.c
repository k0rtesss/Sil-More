...
static bool legendary_area_fit_at(const meta_dungeon_area* area,
    int y0, int x0)
{
...
static void legendary_area_apply_at(const meta_dungeon_area* area,
    int y0, int x0)
{
    int cy = -1;
    int cx = -1;
    ...
    for (int yy = 0; yy < area->record.hgt; yy++)
    {
        for (int xx = 0; xx < area->record.wid; xx++)
        {
            ...
            if (!meta_dungeon_area_cell_at(area, yy, xx, &feat, &color,
                    &role))
            {
                continue;
            }
...
static bool legendary_area_try_place(const meta_dungeon_area* area)
{
    ...
    for (int attempt = 0; attempt < 300; attempt++)
    {
        ...
        log_debug("legendary area: spawned song=%u depth=%u at (%d,%d) size=%ux%u",
            ...
            y0, x0, (unsigned)area->record.wid,
            (unsigned)area->record.hgt);
        return true;
    }
...
static bool place_legendary_area_for_depth(void)
{
    ...
    for (u32b i = 0; i < count; i++)
    {
        if (areas[i].record.depth == p_ptr->depth
            && meta_dungeon_record_is_valid(&areas[i].record,
                areas[i].tile_blob_size))
        {
            eligible++;
        }
    }
