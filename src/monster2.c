...
bool quest_monster_spawn_okay(int r_idx)
{
    ...
    {
        /* Orc captains are reserved for Tulkas' stronghold quest. */
        return false;
    }

    if (p_ptr && r_idx == R_IDX_BELEGWATH)
    {
        /* Belegwath is reserved for Varda's Shadow Bastion quest. */
        if (p_ptr->varda_shadow_restricted ||
    ...
    switch (r_idx) {
        ...
        case R_IDX_ULDOR:
        case R_IDX_ULFANG:
            /* Easterling traitors only spawn in their fortress until completed. */
            if (!p_ptr ||
                quest_get_state(QUEST_ID_MANDOS_TRAITOR) < QUEST_STATE_REWARDED)
            {
                return false;
            }
            return true;
        case R_IDX_MAEGLIN:
            /* Maeglin is locked to Mandos' final quest until completed. */
            if (!p_ptr ||
