...
errr parse_rt_info(char* buf, header* head)
{
    ...
    if (buf[0] == 'C')
    {
        ...
        for (char* tok = strtok(buf + 2, "|"); tok; tok = strtok(NULL, "|"))
        {
            ...
            if (colon)
            {
                ...
                *colon = '\0';
                curse_id = atoi(tok);
                stack_count = atoi(colon + 1);

                if (curse_id >= 0 && curse_id < METAR_CURSE_SLOTS
                    && stack_count > 0 && stack_count <= 255)
                {
    ...
    if (buf[0] == 'H')
    {
        ...
        {
            int w;
            for (w = 0; w < FLAG_WORDS; ++w)
                rt_ptr->heroes[w] = 0xFFFFFFFFu;
            if (FLAG_WORDS > 2)
            {
                for (w = 2; w < FLAG_WORDS; ++w)
                    rt_ptr->heroes[w] = 0;
            }
            return 0;
        }
