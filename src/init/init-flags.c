...
/*
 * Shared flag parsing helpers and debug flag viewer extracted from init1.c.
 */
...
static flag_name info_flags[] = {
    ...
    { "DROP_SUPERB", RF2, RF2_DROP_SUPERB },
    ...
    { "SMT_EOL", UNQ, UNQ_SMT_EOL },
    { "MEL_MAEDHROS", UNQ, UNQ_MEL_MAEDHROS },
    { "WIL_TURIN", UNQ, UNQ_WIL_TURIN },
    { "SMT_CELEBRIMBOR", UNQ, UNQ_SMT_CELEBRIMBOR },
    { "SNG_TURGON", UNQ, UNQ_SNG_TURGON },
    { "MINSTREL", UNQ, UNQ_MINSTREL },
    { "WOVEN_MASTER", UNQ, UNQ_WOVEN_MASTER }
};
...
static const char* rank_name(int lvl)
{
    switch (lvl)
    {
    case 2:
...
static int combined_level(int skill)
{
    static const struct
    {
        ...
        { RHF_MEL_AFFINITY, RHF_MEL_PENALTY },
        { RHF_ARC_AFFINITY, RHF_ARC_PENALTY },
        { RHF_EVN_AFFINITY, RHF_EVN_PENALTY },
        { RHF_STL_AFFINITY, RHF_STL_PENALTY },
        { RHF_PER_AFFINITY, RHF_PER_PENALTY },
        { RHF_WIL_AFFINITY, RHF_WIL_PENALTY },
        { RHF_SMT_AFFINITY, RHF_SMT_PENALTY },
        { RHF_SNG_AFFINITY, RHF_SNG_PENALTY },
        { 0, 0 }
    };

    u32b rhf = p_info[p_ptr->prace].flags | c_info[p_ptr->pcharacter].flags;
    u32b cur = curse_flag_mask();
    int v = 0;

    if (rhf & tbl[skill].aff)
        v++;
    if (rhf & tbl[skill].pen)
        v--;
    if (cur & tbl[skill].aff)
        v++;
    if (cur & tbl[skill].pen)
        v--;
