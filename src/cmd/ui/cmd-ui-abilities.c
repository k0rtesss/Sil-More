...
static void ability_menu_render_score_block(const ability_type* b_ptr,
    int desc_col)
{
    int row = ability_menu_next_row_after_text(desc_col, 3) + 1;
    int info_width = ability_menu_text_width(desc_col, 2);
    ...
    for (int i = 0; i < A_MAX; i++)
    {
        ...
        strnfmt(buf, sizeof(buf), "%s %d x %s = %+d",
            ability_menu_stat_requirement_name(i, info_width <= 30), source,
    ...
    for (int i = 0; i < S_MAX; i++)
    {
        ...
        ability_menu_format_score_weight(weight_buf, sizeof(weight_buf),
            weight);
        strnfmt(buf, sizeof(buf), "%s %d x %s = %+d",
            (info_width <= 30) ? skill_names[i] : skill_names_full[i],
            source, weight_buf, contribution);
        Term_putstr(desc_col + 2, row, -1, TERM_L_GREEN, buf);
        row++;
        any_source = true;
    }
    ...
    int count)
{
    for (int i = 1; i < count; i++)
    {
        ...
        while (j >= 0
            && (ability_requirement_level(entry.b_ptr)
                    < ability_requirement_level(entries[j].b_ptr)
                || (ability_requirement_level(entry.b_ptr)
                        == ability_requirement_level(entries[j].b_ptr)
...
static bool ability_browser_song_bonus_text(const ability_type* b_ptr,
    char* bonus_text, size_t text_size)
{
    int song_skill = b_ptr ? ability_score(b_ptr->skilltype, b_ptr->abilitynum)
                           : ability_menu_current_song_score();
    ...
    const ability_type* b_ptr, int width)
{
    ...
    uses_knowledge = ability_uses_knowledge_points(skilltype, b_ptr->abilitynum);
    ...
    if (b_ptr->lore_req > 0)
    {
        ...
        have_requirement = true;
        strnfmt(buf, sizeof(buf), "Lore: %d (%d now)", need, have);
        ability_desc_add_wrapped(lines, line_count,
            (need <= have) ? TERM_L_GREEN : TERM_L_DARK, buf, width);
    }
    ...
    if (b_ptr->lore_req_lt > 0)
    {
        ...
        have_requirement = true;
        strnfmt(buf, sizeof(buf), "Lore: less than %d (%d now)", limit, have);
        ability_desc_add_wrapped(lines, line_count,
            (have < limit) ? TERM_L_GREEN : TERM_L_DARK, buf, width);
    }
    ...
    for (int i = 0; i < A_MAX; i++)
    {
        ...
        have = ability_requirement_stat_value(i);
        have_requirement = true;
        strnfmt(buf, sizeof(buf), "%s: %d (%d now)",
            ability_menu_stat_requirement_name(i, false), need, have);
        ability_desc_add_wrapped(lines, line_count,
            (need <= have) ? TERM_L_GREEN : TERM_L_DARK, buf, width);
    }
    ...
    for (int i = 0; i < A_MAX; i++)
    {
        ...
        have = ability_requirement_stat_value(i);
        have_requirement = true;
        strnfmt(buf, sizeof(buf), "%s: less than %d (%d now)",
            ability_menu_stat_requirement_name(i, false), limit, have);
        ability_desc_add_wrapped(lines, line_count,
            (have < limit) ? TERM_L_GREEN : TERM_L_DARK, buf, width);
    }
    ...
    for (int i = 0; i < S_MAX; i++)
    {
        ...
        have = ability_requirement_skill_value(i);
        have_requirement = true;
        strnfmt(buf, sizeof(buf), "%s: %d (%d now)", skill_names_full[i],
            need, have);
        ability_desc_add_wrapped(lines, line_count,
            (need <= have) ? TERM_L_GREEN : TERM_L_DARK, buf, width);
    }
    ...
    if (!have_requirement)
        ability_desc_add_line(lines, line_count, TERM_L_GREEN, "None");
        ...
        && prereqs(skilltype, b_ptr->abilitynum))
    {
        if (uses_knowledge)
        {
            int knowledge_cost = ability_purchase_knowledge_cost(skilltype,
                b_ptr->abilitynum);

            strnfmt(buf, sizeof(buf), "Price: %d KP (%ld available)",
                knowledge_cost, (long)p_ptr->knowledge_points);
            ability_desc_add_wrapped(lines, line_count,
                (knowledge_cost <= p_ptr->knowledge_points) ? TERM_L_GREEN
                                                            : TERM_L_DARK,
                buf, width);
...
static void ability_browser_add_score_blocks(ability_browser_desc_line lines[],
    int* line_count, const ability_type* b_ptr, int width)
{
    char buf[160];
    char weight_buf[24];
    bool any_source = false;
    ...
    ability_desc_add_blank(lines, line_count);
    ability_desc_add_line(lines, line_count, TERM_YELLOW, "Contributions");
    ...
    for (int i = 0; i < A_MAX; i++)
    {
        ...
        ability_menu_format_score_weight(weight_buf, sizeof(weight_buf),
            weight);
        strnfmt(buf, sizeof(buf), "%s %d x %s = %+d",
            ability_menu_stat_requirement_name(i, false), source, weight_buf,
            contribution);
    ...
    strnfmt(buf, sizeof(buf), "Current score: %+d",
        ability_score(b_ptr->skilltype, b_ptr->abilitynum));
    ability_desc_add_wrapped(lines, line_count, TERM_WHITE, buf, width);
}
    ...
    int width)
{
    ...
    else
    {
        ...
        {
            strnfmt(status, sizeof(status), "%s. Price %d KP.",
    ...
    ability_browser_add_prerequisites(lines, &line_count, skilltype, b_ptr,
        width);
    ability_browser_add_score_blocks(lines, &line_count, b_ptr, width);
...
static bool ability_browser_activate_choice(int skilltype, int abilitynum)
{
    ...
    if (!p_ptr->have_ability[skilltype][abilitynum])
    {
        ...
        b_ptr = &b_info[ability_index(skilltype, abilitynum)];
        has_stat_skill_prereq = ability_requirements_currently_met(skilltype,
            abilitynum);
        ...
        {
            bell("You do not have enough knowledge to acquire this ability.");
            return false;
        }
    ...
    else
    {
        ...
        else
        {
            if (!ability_requirements_currently_met(skilltype, abilitynum))
            {
                msg_print("Ability requirements are not met.");
                return false;
            }
                ...
                else
                {
                    ...
                    if (!p_ptr->have_ability[skilltype][b_ptr->abilitynum])
                    {
                        ...
                        ability_menu_render_prerequisites_block(skilltype,
                            b_ptr, desc_col);
                        ability_menu_render_score_block(b_ptr, desc_col);
                    }
                    if (has_desc)
                    {
                        if (has_effect
