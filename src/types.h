...
struct object_kind
{
    ...
    bool everseen; /* Tracks whether the item has appeared this run */
};
...
struct quest_type
{
    ...
    byte sequence;  /* Quest position within the Vala's chain (J: field, 1-3) */
    byte quest_flags; /* Flags from F: (global, optional chain, etc.) */
...
struct player_other
{
    ...
    byte narrative_banner_seconds; /* Popup banner hold duration in seconds (0=off) */
...
struct player_type
{
    ...
    byte tulkas_second_spawn_pending; /* Pending spawn for second Tulkas quest giver */
    byte tulkas_morgoth_progress; /* Highest percent of Morgoth HP removed by the player */
    /* Aule quest tracking */
