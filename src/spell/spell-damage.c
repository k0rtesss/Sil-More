...
static void elemental_debug_emit(const elemental_item_debug_info* debug)
{
    ...
    strnfmt(buf, sizeof(buf),
        ...
        debug->roll_cdf * 100.0, debug->q_factor, debug->hurt_factor,
    ...
    int raw_dam, int min_raw, int max_raw, int hp_dam)
{
...

    {
        ...
        {
            int block_chance =
                elemental_shield_block_chance(shield, attack_type);
    ...
    while (total > 0)
    {
        ...
        {
            elemental_message_amount(&candidate, candidate.o_ptr->number,
                amount, o_name, "resisted the attack.",
                "resisted the attack.");
            resisted++;
    ...
    strnfmt(outcome, sizeof(outcome),
        "destroyed=%d(size=%d) resisted=%d(size=%d)", destroyed,
        destroyed_size, resisted, resisted_size);
    ...
    const object_type* o_ptr)
{
    ...
    if (!elemental_attack_allows_candidate_location(attack_type, location,
            index))
    {
        return 0;
    }
...
void fire_dam_pure(int dd, int ds, bool update_rolls, cptr kb_str)
{
    ...
    elemental_attack_affect_multiple_items(GF_FIRE, dam, dd, dd * ds, net_dam);
...
void cold_dam_pure(int dd, int ds, bool update_rolls, cptr kb_str)
{
    ...
    elemental_attack_affect_multiple_items(GF_COLD, dam, dd, dd * ds, net_dam);
