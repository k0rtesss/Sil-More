    ...
    int dif, int typ, u32b flg, int degrees, bool uniform)
{
    ...
    /* Track monster hits and unseen deaths across the projection. */
    projection_monster_state monster_state = {0};
    ...
    if (flg & (PROJECT_GRID))
    {
        ...
        for (i = 0; i < grids; i++)
        {
            ...
            if (projection_affect_feature(
                    who, y, x, gd[i], dam_at_dist[gd[i]], ds, dif, typ))
                notice = true;
        }
    }
    ...
    if (flg & (PROJECT_ITEM))
    {
        ...
        for (i = 0; i < grids; i++)
        {
            ...
            if (projection_affect_object(
                    who, y, x, dam_at_dist[gd[i]], ds, dif, typ))
                notice = true;
        }
    }
    ...
    if (flg & (PROJECT_KILL))
    {
        ...
        for (i = 0; i < grids; i++)
        {
            ...
            if (projection_affect_monster(who, y, x, dam_at_dist[gd[i]], ds,
                    dif, typ, flg, &monster_state))
                notice = true;
        }

        /* Player affected one monster (without "jumping") */
        if ((who < 0) && (monster_state.hit_count == 1)
            && !(flg & (PROJECT_JUMP)))
        {
            /* Location */
            x = monster_state.last_hit_x;
            y = monster_state.last_hit_y;
        ...
        if (monster_state.unseen_death_count)
        {
            /* One monster */
            if (monster_state.unseen_death_count == 1)
            {
            ...
            /* Reset */
            monster_state.unseen_death_count = 0;
        }
    }
