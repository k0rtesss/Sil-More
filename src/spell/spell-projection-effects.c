...
/*
 * Projection effect application helpers: terrain, objects, monsters, and the
 * player.
 */
    ...
    int typ, u32b flg, projection_monster_state* state)
{
        ...
        if (cave_info[y][x] & (CAVE_VIEW))
        {
            ...
            if (r_ptr->flags3 & (RF3_HURT_LITE))
            {
                ...
                if ((who < 0) && (light_level > 2))
                {
                    ...
                    /* If dif >= 0, this is Song of Trees (dif contains song
                     * score), otherwise use Will */
                    if (dif >= 0)
                        skill_to_use = dif;
                    else
                    ...
                    light_check_result = skill_check(PLAYER, skill_to_use,
                        will_resistance + 5
                            + distance(p_ptr->py, p_ptr->px, y, x),
                        m_ptr);

                    /* Stun is applied when monster FAILS Will save (result > 0
                     * means player wins) */
                ...
                {
                    /* Light level too low or not player-caused - no damage or
                     * stun */
                    dam = 0;
                }
            }
    ...
    if ((dam > 0) && m_ptr->ml)
    {
        ...
        update_combat_rolls2(
            combat_dd, combat_ds, dam, -1, -1, 0, 0, typ, false);
    }
    ...
    if (who > 0)
    {
        ...
        if (m_ptr->hp <= 0)
        {
            /* Song of Trees: trolls slain by radiant light crumble into rubble
             * (Kemenrauko-style). */
    ...
    else
    {
        ...
        /* Any player-sourced effect that touches Morgoth breaks Nienna's
         * charge */
     ...
     * make noise. -LM-
     */
    if ((state->hit_count == 1) && (who <= 0) && (dam))
    {
        stealth_score -= 0;
    }
    ...
    int who, int y, int x, int dd, int ds, int dif, int typ)
{
    ...
    case GF_BOULDER:
    {
        ...
        {
            crit_bonus_dice = crit_bonus(
                hit_result, 100, &r_info[0], S_ARC, true, m_ptr, NULL);
            total_dd = dd + crit_bonus_dice;
            total_ds = ds;
