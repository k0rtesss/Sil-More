...
errr rd_extra(void)
{
    ...
    for (i = 0; i < S_MAX; i++)
    {
        for (j = 0; j < ABILITIES_MAX; j++)
        {
            ...
            {
                log_trace("Load: Special ability %d loaded with value %d", j,
    ...
    if (savefile_version_at_least(0, 9, 5, 8))
    {
    ...
    log_debug("rd_extra: reading quest block (byte_ofs=%u) version=%u.%u.%u",
        (unsigned)load_byte_offset, (unsigned)sf_major, (unsigned)sf_minor,
        (unsigned)sf_patch);
    ...
    {
        note(format(
            "Invalid quest marker 0x%02X (expected 0x51). Savefile too old (< 0.8.9).",
            quest_marker));
        return (-1);
    }
    ...
    if (savefile_version_at_least(0, 9, 5, 8))
    {
        for (int vi = 0; vi < VALA_MAX; vi++)
            rd_byte(&p_ptr->vala_quest_stage2[vi]);
        for (int vi = 0; vi < VALA_MAX; vi++)
            rd_byte(&p_ptr->vala_quest_stage3[vi]);
    }
    else
    {
        for (int vi = 0; vi < VALA_MAX; vi++)
    ...
    int quest_reserved_len = savefile_version_at_least(0, 9, 5, 8)
        ? (int)N_ELEMENTS(p_ptr->quest_reserved)
        : (savefile_version_at_least(0, 9, 1, 3) ? 15 : 12);
    for (int qi = 0; qi < quest_reserved_len
        && qi < (int)N_ELEMENTS(p_ptr->quest_reserved); qi++)
    {
        rd_byte(&p_ptr->quest_reserved[qi]);
    }
    for (int qi = quest_reserved_len;
        qi < (int)N_ELEMENTS(p_ptr->quest_reserved); qi++)
    {
    ...
    if (savefile_has_skeleton_notes)
    {
        ...
        {
            note(format("Invalid skeleton note marker 0x%02X",
                skeleton_marker));
            return (-1);
        }
    ...
    if (savefile_has_partition_meta)
    {
        ...
        {
            note(format("Invalid partition meta marker 0x%02X",
                partition_marker));
            return (-1);
        }
    ...
    if (savefile_has_hint_messages)
    {
        ...
        for (int mi = 0; mi < count; ++mi)
        {
            ...
            if (savefile_has_hint_message_meta)
            {
                ...
                {
                    rd_string(meta.cue_dists[cue],
                        sizeof(meta.cue_dists[cue]));
                    rd_string(meta.cue_dirs[cue],
                        sizeof(meta.cue_dirs[cue]));
                }
            }
    ...
    log_info("LOAD: min_depth_counter=%d, calculated min_depth()=%d",
        min_depth_counter, min_depth());
...
errr rd_randarts(void)
{
    ...
    if (!(begin == 0 || begin == z_info->art_norm_max))
        header_valid = false;

    log_debug(
        "randarts header: begin=%u artefact_count=%u art_norm_count=%u (limits: art_max=%d art_norm_max=%d) valid=%d",
        begin, artefact_count, art_norm_count, z_info->art_max,
        z_info->art_norm_max, header_valid ? 1 : 0);
    ...
    if (!p_ptr->is_dead || runtime_cli_wizard())
    {
        ...
        {
            if (artefact_count > z_info->art_max)
                note(format("Too many artefacts in save (%u > %d)",
                    artefact_count, z_info->art_max));
            else
                note(format("Too many normal artefacts in save (%u > %d)",
                    art_norm_count, z_info->art_norm_max));
            log_error("randarts mismatch: begin=%u art=%u/%d norm=%u/%d",
                begin, artefact_count, z_info->art_max, art_norm_count,
                z_info->art_norm_max);
            return (-1);
        }
