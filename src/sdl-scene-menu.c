...
typedef struct sdl_menu_font_cache {
    TTF_Font* font;
    int pixel_height;
    int style_signature;
    char path[1024];
...
static sdl_menu_font_cache g_sdl_menu_font_cache;
...
static const char* sdl_menu_font_path(void)
{
...
static int sdl_menu_font_signature(void)
{
    int signature = 0;
...
static void sdl_menu_apply_font_settings(TTF_Font* font)
{
...
static void sdl_menu_font_clear(void)
{
    if (g_sdl_menu_font_cache.font)
    {
        TTF_CloseFont(g_sdl_menu_font_cache.font);
        g_sdl_menu_font_cache.font = NULL;
    }

    g_sdl_menu_font_cache.pixel_height = 0;
    g_sdl_menu_font_cache.style_signature = 0;
    g_sdl_menu_font_cache.path[0] = '\0';
}

static TTF_Font* sdl_menu_font_for_height(int pixel_height)
{
    const char* font_path = sdl_menu_font_path();
    int style_signature = sdl_menu_font_signature();

    if (pixel_height <= 0)
        return NULL;

    if (g_sdl_menu_font_cache.font
        && g_sdl_menu_font_cache.pixel_height == pixel_height
        && g_sdl_menu_font_cache.style_signature == style_signature
        && streq(g_sdl_menu_font_cache.path, font_path))
    {
        return g_sdl_menu_font_cache.font;
    }

    sdl_menu_font_clear();

    g_sdl_menu_font_cache.font = TTF_OpenFont(font_path, pixel_height);
    if (!g_sdl_menu_font_cache.font)
    {
        log_warn("SDL menu font load failed for '%s': %s", font_path,
            SDL_GetError());
        return NULL;
    }

    sdl_menu_apply_font_settings(g_sdl_menu_font_cache.font);
    g_sdl_menu_font_cache.pixel_height = pixel_height;
    g_sdl_menu_font_cache.style_signature = style_signature;
    SDL_strlcpy(g_sdl_menu_font_cache.path, font_path,
        sizeof(g_sdl_menu_font_cache.path));
    return g_sdl_menu_font_cache.font;
}
...
static int sdl_menu_font_size_logical(const sdl_view* view)
{
    ...
    logical_size = 24;
    if (canvas_h > 0 && canvas_h < sdl_menu_scale_px(420.0f))
        logical_size = 16;
    else if (canvas_h > 0 && canvas_h < sdl_menu_scale_px(540.0f))
        logical_size = 18;
    else if (canvas_h > 0 && canvas_h < sdl_menu_scale_px(700.0f))
        logical_size = 20;
    ...
    int item_gap, int current_y, byte accent_attr)
{
    ...
    if (!(scene->flags & APP_MENU_SCENE_FLAG_PLAIN) && row->key[0])
    {
        ...
        sdl_menu_render_text(font, (float)clip_rect->x, (float)current_y,
            sdl_menu_color(scene->accent_attr), row->key);
    ...
    if (row->label[0])
        sdl_menu_render_text(font, (float)label_x, (float)current_y, color,
            row->label);
    if (row->meta[0])
        sdl_menu_render_text(font, (float)meta_x, (float)current_y, color,
            row->meta);
}
...
static void sdl_menu_render_tabs(TTF_Font* font, const app_menu_scene* scene,
    const SDL_Rect* clip_rect, int line_h, int pill_gap, int pill_pad_x,
    int pill_pad_y)
{
    int cursor_x;
    u16b i;
    ...
    const app_menu_scene* scene)
{
    ...
    font = sdl_menu_font_for_height(pixel_height);
    if (!font)
        return false;
    ...
    if (scene->tab_count > 0)
    {
        ...
        sdl_menu_render_tabs(font, scene, &tabs_clip, line_h, pill_gap,
            pill_pad_x, pill_pad_y);
        current_y += tabs_h;
        if (header_h > 0)
    ...
    if (scene->title[0])
    {
        sdl_menu_render_text(font, (float)left_clip.x, (float)current_y,
            sdl_menu_color(scene->title_attr), scene->title);
        current_y += line_h + line_gap;
    }
    if (scene->subtitle[0])
    {
        sdl_menu_render_text(font, (float)left_clip.x, (float)current_y,
            sdl_menu_color(scene->subtitle_attr), scene->subtitle);
        current_y += line_h + line_gap;
    }
    if (has_columns)
    {
    ...
    if (body_h > 0 || rows_h > 0)
    {
        ...
        {
            sdl_menu_render_text(font, (float)left_clip.x, (float)current_y,
                sdl_menu_color(scene->body_lines[i].attr),
                scene->body_lines[i].text);
            current_y += line_h + line_gap;
        }
        ...
        if (row_start > 0)
        {
            sdl_menu_render_text(font,
                ...
                (float)current_y, sdl_menu_color(scene->accent_attr), "^");
        }
        ...
        for (i = 0; i < (u16b)row_visible; i++)
        {
            ...
            sdl_menu_render_row(font, scene, row, &column_clip, line_h, item_gap,
                current_y, scene->accent_attr);
            current_y += line_h + line_gap;
        }
    ...
    if (has_detail)
    {
        ...
        if (scene->detail_title[0])
        {
            sdl_menu_render_text(font, (float)right_clip.x, (float)detail_y,
                sdl_menu_color(scene->detail_title_attr), scene->detail_title);
            detail_y += line_h + line_gap;
        }
        for (i = 0; i < scene->detail_line_count; i++)
        {
            sdl_menu_render_text(font, (float)right_clip.x, (float)detail_y,
                sdl_menu_color(scene->detail_lines[i].attr),
                scene->detail_lines[i].text);
            detail_y += line_h + line_gap;
        }
        SDL_SetRenderClipRect(g_state.renderer, NULL);
    ...
    if (has_footer)
    {
        sdl_menu_render_footer(font, scene, &footer_clip, line_h, pill_gap,
            pill_pad_x, pill_pad_y);
    }
