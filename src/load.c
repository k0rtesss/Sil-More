...
static bool savefile_has_revenge_fields = false;
...
static errr rd_extra(void)
{
    ...
    // Reserved block: legacy status bytes plus branch-added state fields.
    {
        ...
        if (savefile_has_revenge_fields)
        {
    ...
    if (savefile_version_at_least(0, 9, 7, 3))
    {
        rd_u16b(&p_ptr->manwe_deception_flags);
        p_ptr->manwe_deception_flags &= MANWE_DECEPTION_FLAG_MASK;
    }
    else
    {
