    ...
    int total_pages, bool include_header)
{
    ...
    case 4:
    {
        ...
        put_role(ROLE_BODY, "- Fire melts solid ice into shallow water. Cold freezes water into solid ice.", row++, col);
...
#undef HELP_MISC_COMMAND
    ...
    case 9:
    {
        ...
        put_role(ROLE_TERM, "Pack", row, x); x += 4;
        put_role(ROLE_BODY, " apparel: ", row, x); x += 10;
        put_role(ROLE_GOOD, "no volume", row, x); x += 9;
        put_role(ROLE_BODY, "; ", row, x); x += 2;
        put_role(ROLE_TERM, "Harness", row, x); x += 7;
        put_role(ROLE_BODY, " gear keeps volume; ", row, x); x += 20;
        put_role(ROLE_GOOD, "no slot cap", row, x); x += 11;
        put_role(ROLE_BODY, ".", row, x);
        row++;
        x = col;
        put_role(ROLE_BODY, "- ", row, x); x += 2;
