...
void runtime_dungeon_process_world(void)
{
    ...
    if (o_ptr->tval == TV_LIGHT)
    {
        ...
        if (player_light_has_fuel(o_ptr))
        {
            ...
            {
                /*
                 * Small caves: double fuel drain only while standing in the
                 * actual CA-blob cave area (not merely anywhere in the
                 * partition).
                 *
                 * CA blobs are generated as (dark) "room" grids;
                 * corridors/links are not.
                 */
