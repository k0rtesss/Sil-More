...
def check_flag_tables(tr_defines: set[str]) -> list[str]:
    errors: list[str] = []
    ...
    for path in TABLE_PATHS:
        ...
        for match in TABLE_RE.finditer(text):
            ...
            if table_set != const_set:
                ...
                errors.append(
                    f"{rel}:{line}:{col}: table set TR{table_set} mismatches {constant}"
                )
...
def main() -> int:
    ...
    errors = check_flag_tables(tr_defines)
