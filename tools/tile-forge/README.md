...
A local editor for composing **any existing PNG artwork** into floor, wall, and chasm autotiles. It reads the current Sil-More atlas and exports new files through the browser. Source images remain unchanged. The accompanying SDL changes connect arbitrary floor–floor, floor–wall, and wall–wall materials independently of atlas coordinates. Game chasms keep their dark fill and resolve irregular connections inside the chasm cells, without an overlay on neighboring terrain.
...
The mixed preview is an art workbench, not a pixel-identical reproduction of the SDL renderer. Runtime uses discrete, one-sided material contours and separate wall contact shading; the editor offers adjustable contour, dither, and palette controls. The 47-shape gallery and atlas exports describe the upper/lower pair selected in the sidebar; they do not encode every mixed neighborhood in one binary mask.
...
`src/sdl/render/sdl-material-edge.c` resolves one source material per pixel within a three-pixel border. Same-plane floor and wall materials use quarter-cell contours: cardinal cuts, concave diagonal notches, and sparse world-coordinate variation with matching endpoints. A stable artwork order makes one side recede; it avoids two opposing outlines and draw-order-dependent junctions. Source texels retain their local coordinates, orientation and colors. No translucent texture strips are stacked at corners.

Material identity comes from the actual artwork: sorted, deduplicated floor variant families and normalized wall coordinates. Reordering variants or using two style IDs for the same art creates no boundary. A shore override is classified by its displayed artwork, not the original style. Atlas coordinates provide a deterministic fallback priority, not a claim about the physical height of a material.

Walls keep their own pixels. A separate, bounded directional shade describes exposed wall faces and floor contact; floors never donate texture into solid walls. Diagonal material contacts require two known intervening cells on the same plane. Doors retain their complete authored silhouettes. Native snow/dirt and basalt/dirt contours take precedence when successfully rendered; unavailable atlases fall back to the generic contour. Their masks use the displayed floor artwork after shore overrides, so stored dirt that appears as snow beside ice cannot create an internal dirt seam. Only visible dirt opens a contour; unknown terrain and blocked diagonal contacts do not. Liquids, hazards, bridges and chasms retain their dedicated passes.

The pass runs below objects/actors and fixtures, with no gameplay RNG, per-cell texture creation, or new asset loading. Known terrain alone can supply donor pixels. `map_info_terrain()` shares normal terrain selection but stops before object/actor perception; Boolean edge queries stop at the first differing source. Hallucination skips the pass. Terrain-source invalidation propagates artwork and visibility changes to adjacent cached cells, including removal of a boundary. Equal coverage pixels are merged into rectangles and submitted through SDL's sprite/fill paths, avoiding expensive general triangle rasterization on software renderers. Transparent source texels use the normal black terrain backing instead of exposing the previous material.

### Runtime chasm connections

Chasms are cracks in the mountain. `sdl-chasm-transition.c` uses eight neighbors and quarter-cell contours to shape the existing dark fill. It samples the actual known terrain next to the chasm into irregular 2-4px margins **inside the chasm cell**. This works with original and Verdant floors and walls without pair-specific atlases. The middle stays visibly open, including one-cell cracks. Concave corners require connected void on both intervening sides; unknown terrain cannot supply donor pixels.

Neighboring floors and walls keep their complete artwork: no raised rim, bright lip or border is painted over them. Doors supply their floor underlay, actors are never donors, and bridges retain their deck above the connected chasm underlay. Liquid donors use their current surface frame and shoreline layers. The original `sdl-chasm-edge.c` overlay remains removed. The editor's optional chasm-rim authoring mode is separate from the runtime presentation.

Partition materials are restored to the policy before `405da557`: `P:CHASM` selects style 30 (Purple2 / jagged purple crystal); bridges use the ordinary depth pool. The regression had selected style 40 (Flagstone) for both. New levels use the restored rules; saved cell styles remain intact. Chasms inside authored vaults continue to use the vault's own palette.
...
4. **Height and material layering:** Value separation and an authored descending face can describe a drop. A continuous bright rim is an art-direction choice, not a requirement; Sil-More uses dark chasm fills with internal terrain-aware contours. Height-aware landscape layering is a useful larger-scale analogy; it is not the exact implementation here: [Unreal Engine Landscape Quick Start](https://dev.epicgames.com/documentation/unreal-engine/landscape-quick-start-guide-in-unreal-engine).
...
### September 2026 runtime review

The recent implementation was inspected at `b830b1b8`, `405da557` and `7224ad50`. Its dedicated binary atlases already used Verdant's 47 normalized neighborhoods. The generic runtime path instead layered eight rectangular, alpha-faded donor patches. These are different algorithms: normalizing the atlas masks did not constrain generic runtime corners. Style-number identity also introduced false seams between Brickwork and DirtCave, which share their source art. Finally, generic strips could cover already-rendered snow/dirt contours. The current resolver addresses these problems without generating pairwise atlases for every handcrafted material.

The common engine pattern is to match topology first, then select artwork. [Tiled](https://doc.mapeditor.org/en/stable/manual/terrain/) documents corner, edge and mixed terrain sets, including reduced 47-tile blobs. [Godot](https://docs.godotengine.org/en/stable/tutorials/2d/using_tilesets.html#creating-terrain-sets-autotiling) separates terrain identities and peering bits from tile artwork. [Unity Rule Tile](https://docs.unity3d.com/Packages/com.unity.2d.tilemap.extras@4.0/manual/RuleTile.html) uses ordered neighborhood rules and selectable output sprites. Our runtime ownership rule is a practical adaptation for existing arbitrary tiles, not an implementation claimed to be shared by these engines.

Inspection of local VerdantForge's `normalise`, `insideMask`, `rimPixels`, `makeJitter` and `wallTile` confirmed the useful foundations: irrelevant diagonals are removed, four quarters resolve corners, rims must respect neighboring occupancy, and wall tops/faces have different lighting. Its binary mask cannot encode a junction of arbitrary materials; custom transitions also use one source variant, and periodic local jitter can repeat visibly. The runtime uses actual selected variants and world-coordinate contour variation while retaining dedicated authored atlases where available. It does not invent a tall wall face from flat floor art or rotate artwork with directional lighting.
...
- Keep natural chasms free of uniform constructed borders. The current connection contours stay inside the chasm cell; future rock-face artwork should preserve that ownership and be reviewed at game zoom.
...
The JavaScript checks cover mask completeness, pixel preservation, deterministic geometry, palette/alpha behavior, shared-edge continuity, and ZIP integrity. The production SDL chasm harness exhausts all 256 neighbor masks and verifies that known chasms do not alter any pixels of eight original/imported floor and wall artworks. It checks internal contour ownership, clear centers and thin connections, visibility, animated liquid donors, bridge underlays, excluded foreground sprites, and incremental discovery/removal/pan redraw against full repaint. Its connected-crack preview is `scripts/output/chasm-edges-check/connected-chasms.png`. The material suite also exhausts 256 floor and 256 wall neighborhoods for exact palette ownership, verifies three-material texel coordinates, equal-artwork style aliases, shore overrides, blocked diagonal contacts, native-atlas precedence/fallback, and repeated-draw determinism. It writes `material-edges-check/contact-sheet.png` and `material-edges-check/winding-stock-materials.png` under `scripts/output/`. It also runs the existing idle-animation regressions and writes an eight-material comparison to `scripts/output/chasm-edges-check/materials.png`. Browser verification covered presets, numeric source picking, embedded project save/open with a drawn replacement, and an actual generated ZIP with all 256 slots checked against the compact sheet.
