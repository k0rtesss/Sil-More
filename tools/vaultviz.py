...
"""
Vault visualization tool for Sil-More.
...
# Coordinates were derived from the old graf preference data.
VAULT_SYMBOL_TILES: dict[str, tuple[int, int]] = {
    # Monsters (from the old graf preference R: entries)
...
def render_vault_png(
    ...
    *,
    ...
    # Feature tiles from the old graf preference data (fixed, not style-driven).
    FEAT_SUNLIGHT = 9
    FEAT_GLYPH = 3
    FEAT_CHASM = 2
    FEAT_RUBBLE = 49
...
def build_parser() -> argparse.ArgumentParser:
    ...
    render_p.add_argument("--graf-prf", default=DEFAULT_GRAF_PRF, help="Optional graf*.prf path for feature tiles")
    ...
    sheet_p.add_argument("--graf-prf", default=DEFAULT_GRAF_PRF, help="Optional graf*.prf path for feature tiles")
