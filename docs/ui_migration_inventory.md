...
| `src/main-sdl.c`, `src/sdl-layout.c`, `src/sdl-render.c`, `src/sdl-scene.c`, `src/sdl-scene-dungeon.c`, `src/sdl-touch.c`, `src/sdl-story-font.c`, `src/sdl-config.c`, `src/sdl-sound.c`, `src/pane.c` | early frontend-only | UI4 | SDL scene/render/layout ownership belongs here, not in gameplay modules. |
...
| `src/z-term.c`, `src/platform-config.h`, `src/platform-input.h`, `src/platform-story-font.h`, `src/platform-time.[ch]`, `src/platform-io.h` | early frontend-only | UI7 | Legacy compatibility and neutral platform boundary surface. Keep UI host/platform ownership here rather than reintroducing SDL-facing headers into core modules. |
