...
- The unstable-port milestone is structurally landed, but the overall refactor is not finished completely yet:
  ...
  - `WP71C` is now landed in the working tree: `src/fs/savefile-name.[ch]` owns savefile/name normalization, and the semantic birth flow owns the active character-naming UI
...
- `Phase 2` is substantially landed through `WP14`, with both `WP10` and `WP11` now structurally complete:
  ...
  - `src/level-generation/level-generation.c` is now reduced to the intended top-level driver/orchestration layer (`cave_gen()`, `gates_gen()`, `spawn_niena_morgoth_hall()`, `unring_a_bell()`, and `generate_cave()`), matching the role that `unstable` kept in its final `level-generation.c`.
...
- `WP71C`: completed in the working tree on 2026-03-25; `src/fs/savefile-name.[ch]` now owns `process_player_name()`, the semantic birth flow owns the active player-name prompt path, and `files.c` no longer owns the player-name/savefile prompt path.
...
- `WP71C`: extract savefile/name normalization into `src/fs/savefile-name.[ch]`; keep any naming prompt logic with the active birth UI only if that loop still exists outside the semantic flow.
