...
**Forge.** The entrance belongs to Ondotur. Its guarded ladder leads to a handcrafted level 23 containing Nambatur, chests and Orodruth, the mighty forge with three uses and its existing +7 Smithing bonus. With Utumno enabled, ordinary forge allocation cannot create Orodruth. The forge records Morgoth and Sauron's work on Grond; it does not award a second Grond.

**Living landscape (September 2026 revision).** Utumno's three procedural networks use broader channels and larger basins (radii 5–12). Irregular glacial shelves grow around exterior lava flows, opening unworked granite while preserving constructed corridors, vaults, fixtures and existing destinations. Map sizing, the Smaller level size setting, ordinary generation/population passes and final walking-route validation still apply.

Both Utumno floors run the existing persistent environment simulation. Direct lava contact melts ice through the melting-ice stage into water; that water quenches adjacent lava into walkable stone. Cold can refreeze water away from lava, while the existing finite reservoirs rise and recede. Utumno uses the configured per-pulse change budget for multiple thermal fronts instead of a single thermal tile across the map. Occupied tiles, critical objects and mandatory routes retain the existing protections. The entrance and forge opt in only their authored ice, water and lava cells: masonry, dry galleries, bridges, chests, stairs and Orodruth stay protected. No save-format change is required; new shelves and vault designs appear on newly generated floors.

The 49×31 entrance combines a receiving gate, ore store, wardens' chamber, ice-choked furnace flue, breached cistern and guarded hoist hall. The 61×55 forge has a screened arrival, three furnace mouths sharing a lava race, a pillared anvil hall and backed hearth, two quenching workshops, storage rooms and a screened private stair. Dry service galleries connect the work areas and rewards. Vault templates use `{` for actual ice, alongside `_` water and backtick lava; `%` remains quartz.

**Architecture of the forge route.** The two templates use the same design principles as the Cat Fortress (guarded courts with flanking circulation), the Chambers of Thu (separate rooms with recognisable purposes), and Morgoth's throne room (a public approach and a screened private route). They remain authored vaults 523 and 524. These rooms are an imagined working complex, not a reconstruction of a Tolkien floor plan.

- **The Doors of Grond's Forge, 25 by 41:** a frost-covered receiving court admits heavy loads through a broad northern gate, with loading and service gates on the flanks. An ore store and a wardens' store flank the court; the western goods aisle passes a broken hot flue, while the eastern gallery serves a cistern. Ondotur occupies the pillared hoist hall beyond the inner gate. Its ladder descends beside the old load-handling space. Both side aisles join the rear of the hall, allowing a flanking approach and retreat around the piers. The two existing chests occupy the stores.
- **The Forge of Grond, 41 by 57:** the sheltered northern landing turns into a receiving aisle between the ore bay and pattern strongroom. Three western fireboxes draw heat from a lava race, with a dry stoking gallery and a slag bay behind them. The broad central floor gives Nambatur, the anvil and large workpieces room to move; roof piers and stone tool benches break sightlines. A backed hearth burns behind Orodruth. On the cool eastern side, a reservoir supplies quenching troughs beside the finished-work store. A rear service crossway leads through a separate antechamber to Morgoth's private stair.
- **Encounter and access:** the furnace gallery and quenching room reconnect to the working floor at both ends. All three forge chests, Orodruth and the return stair have routes without crossing lava, water, rubble or secret doors. The arrival has no direct line of sight to Nambatur. Existing guardians, chest counts, forge uses and the one-way expedition remain unchanged; combat balance still needs playtesting.
- **Generation contract:** level 22 now checks an actual walking route from arrival to the mandatory ladder. The shared room-connectivity flood permits leaps and treats vault interiors abstractly, so it could accept an approach cut off by lava. Utumno rejects that candidate and generates another layout. `scripts/check_utumno_generation.py` checks both template circulation and production access, including Smaller level size, Meager loot and the complete return route.
